				...
				<main className='gymPage-section flex-column '>
					<form className='flex gap-xl' onSubmit={handleWorkoutSubmit}>
						<select
							...
							{muscleGroup.map((muscleGroupe, id) => {
								return (
									<option key={muscleGroupe.id} value={muscleGroupe.value}>
										{muscleGroupe.value}
									</option>
								);
							})}
					...
					<section className='grid gap-l'>
						{React.Children.toArray(
							state.workouts! &&
								state.workouts.map((muscleGroupe: any, id: number) => (
									<div className='gymPageCard-shadow ' key={muscleGroupe.id}>
										...
										<Link to={muscleGroupe.toLowerCase()} state={muscleGroupe}>
											<img
												src={`${process.env.PUBLIC_URL}/img-svg/img/${muscleGroupe}.png`}
												alt={`${muscleGroupe}`}
